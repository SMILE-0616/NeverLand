...
public class Book : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        {
            leftInteractor.selectEntered.AddListener(OnLeftSelectEntered);
            leftInteractor.selectExited.AddListener(OnSelectExited);
        }
        if (rightInteractor != null)
        {
            rightInteractor.selectEntered.AddListener(OnRightSelectEntered);
            rightInteractor.selectExited.AddListener(OnSelectExited);
        }
    }
    ...
    private void OnDisable()
    {
        if (leftInteractor != null)
        {
            leftInteractor.selectEntered.RemoveListener(OnLeftSelectEntered);
            leftInteractor.selectExited.RemoveListener(OnSelectExited);
        }
        if (rightInteractor != null)
        {
            rightInteractor.selectEntered.RemoveListener(OnRightSelectEntered);
            rightInteractor.selectExited.RemoveListener(OnSelectExited);
        }
    }

    private void OnLeftSelectEntered(SelectEnterEventArgs args)
    {
        if (interactable) DragLeftPageToPoint(transformPoint(args.interactor.transform.position));
    }

    private void OnRightSelectEntered(SelectEnterEventArgs args)
    {
        if (interactable) DragRightPageToPoint(transformPoint(args.interactor.transform.position));
    }
